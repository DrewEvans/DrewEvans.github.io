...
router.get("/photographers/tags", (req, res) => {
	let tags = []; //empty array

	data.photographers.forEach((photographer) => {
		//push all tags from every photographer into tags array
...
//all media
router.get("/photographers/media", (req, res) => {
	const media = [
		{
			id: 342550,
			photographerId: 82,
...
//request data from photographer with unique ID
router.get("/photographers/:id", (req, res) => {
	//filter photographer with id requested from the client side
...
//request photographer media from photographer that matches requested ID
router.get("/photographers/:id/media", (req, res) => {
	//find photographer with id requested from the client side

	const photographerMedia = data.media.filter(function (photographerMedia) {
		return photographerMedia.photographerId == req.params.id;
	});
